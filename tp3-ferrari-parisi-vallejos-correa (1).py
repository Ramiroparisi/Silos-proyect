...
def validarFecha():
  ...
  flag = True
  fla = True
  print(c.amarillo+"La fecha a ingresar podra ser mayor o igual a la fecha de hoy"+c.resetear)
  
  while flag:
    try:
      fech = input(c.verde+"Ingresa una fecha en el formato DD/MM/AAAA: "+c.resetear)
...
def existe_productos_activos_codigo(x):
    global AF_PROD,AL_PROD
    t=os.path.getsize(AF_PROD)
    AL_PROD.seek(0)
    while AL_PROD.tell()<t:
        pos= AL_PROD.tell()
        reg=pickle.load(AL_PROD)
        if reg.cod.strip()==x and reg.est=='A':
            return True
    return False
...
def alta_productos():
    ...
    while rta=='S':
        ...
        pro=input("\nIngrese el producto [hasta 15 caracteres] o escriba salir, para salir sin hacer cambios → ")
        ...
        if pro!="SALIR":
            ...
            if busca_nombre(AF_PROD,AL_PROD, pro) == -1:
                ...
                print(c.rojo + "⚠ El producto ya se encuentra registrado. Verifique el estado\n" + c.resetear)
        ...
        if pro=="SALIR":
            ...
            while rta != "S" and rta != "N":
                rta= input(c.rojo + "⚠ Por favor, solo S para Si o N para No:" + c.resetear).upper()
...
def busco_prod_cam_editable(x):
    ...
    if fin==0:
        ...
        while AL_OP.tell() < fin and ban==False:
            Regop=pickle.load(AL_OP)
            if int(Regop.cod)==int(x) and not Regop.est=='F' and not Regop.est=='R' and Regop.fecha==hoy:
                ban=True
                return ban
        return False
...
def baja_prod():
    ...
    while rta=='S':
        os.system('cls')
        print(c.bold + c.cyan + "                           Opcion B - Baja de productos " + c.resetear )
        print("                         __________________________________\n")
        global AF_PROD, AL_PROD
        t=os.path.getsize(AF_PROD)
        if not productos_activos:
            print(c.verde + "\n ⚠ No hay productos para Borrar" + c.resetear)
            os.system('pause')
        else:
            mostrar_productos()
            cantp=cant_registros(AF_PROD,AL_PROD)
            cod=input(f"\nIngrese el codigo de producto a borrar o 0 para no modificar nada → ")
            while not existe_productos_activos_codigo(cod) and cod!="0":
                cod=input(c.rojo + f"Ingrese el codigo de producto a borrar o 0 para no modificar nada → " + c.resetear)
            cod=int(cod)
            if cod!=0:
                #Busco si ya ingresó un camión con ese producto
                if busco_prod_cam_editable(cod):
                    print(c.rojo + "⚠ Un camion ya ha ingresado con este producto, por lo tanto no se puede borrar."+ c.resetear)
                    os.system('pause')
                else:
        ...
        if cod==0:
            ...
            rta= input("Desea Borrar otro producto? S-si   N-no: ").upper()
...
def consulta_productos():
    os.system('cls')
    print(c.bold + c.cyan + "                           Opcion C - Consulta de productos " + c.resetear )
    print("                         __________________________________\n")
    print(c.verde + "\nLa Consulta de Productos Muestra los Productos Activos.\n" + c.resetear)
    mostrar_productos()
    print()
    os.system('pause')
...
def modifica_prod():
    ...
    if t==0:
        ...
        while rta=='S':
            os.system('cls')
            print(c.bold + c.cyan + "                        Opcion M - Modificación de productos " + c.resetear )
            print("                         __________________________________\n")
            mostrar_productos_all()
            cantp=cant_registros(AF_PROD,AL_PROD)
            cod=input(f"\nIngrese el codigo de producto a Modificar o 0 para salir sin hacer cambios → ")
            while validaRangoEntero(cod,0,cantp):
                cod=input(c.rojo + f"ERROR. Ingrese el codigo de producto a modificar o 0 para salir sin hacer cambios  → " + c.resetear)
            cod=int(cod)
            if cod!=0:
                #Busco si ya ingresó un camión con ese producto
                if busco_prod_cam_editable(cod):
                    print(c.rojo + "⚠ Un camion ya ha ingresado con este producto, por lo tanto no se puede Modificar."+ c.resetear)
                    os.system('pause')
                else:
                    pos=busca_codigo(AF_PROD,AL_PROD,cod)
                    rPro=prod()
...
def mostrar_productos(): # MUESTRA PRODUCTOS ACTIVOS
    ...
    if t==0:
        ...
        if cont==0:
            print(c.rojo + "⚠ No hay productos activos" + c.resetear)
...
def entrega_cupos():
    ...
    while rta=='S':
        ...
        if os.path.getsize(AF_PROD)==0:
            print(c.rojo + f"⚠ No se pueden entregar cupos si no hay productos cargados. Cargue algún producto" + c.resetear)
        else:
            rOp.pat=validarPatente()
            if rOp.pat!="0":
                
                if patente_hoy(rOp.pat):
                    print(c.rojo+"\n⚠ Cupo ya otorgado\n"+c.resetear)
                    os.system('pause')
                else:
                    mostrar_productos()
                    print()
                    rOp.cod=input("Ingrese el código del producto o 0 para salir → ")
                    while not existe_productos_activos_codigo(rOp.cod) and rOp.cod!="0":
                        rOp.cod=input(c.rojo + "ERROR. Ingrese el código del producto o 0 para salir → " + c.resetear)
                    rOp.cod=int(rOp.cod)
                    if rOp.cod!=0:
                        rOp.pat=rOp.pat.ljust(7)
...
def validarPatente():
    f=True
    while f==True:
        pat=str.upper(input(c.verde + f"Ingrese patente en formato ABC123 o AB123CD  [0 para cancelar la operación]: → "+ c.resetear))
        if pat!="0":
            x=len(pat)
            if x==6:
...
def patente_hoy(pat):
    ...
    while AL_OP.tell()<t:
        ...
        if reg.pat.strip()==pat and reg.fecha==hoy:
            return True
    return False
...
def bruto():
    ...
    while rta=='S':
        ...
        if rta=='N':
            ...
            rta= input("Desea cargar otro camión? S-si   N-no: ").upper()
            while rta != "S" and rta != "N" and rta!="s" and rta!= "n":
                print()
...
def archivos():
    ...
    while flag==True:
        ...
        if opcion == "1":
            ...
            #dicotomica()
            # cod=input("Ingrese un codigo -> ").ljust(2)
            # a=BuscaDico(cod)
            # AL_RUBRO.seek(a)
            ...
            prod=input("Ingresa un coodigo de producto -> ")
            a=existe_productos_activos_codigo(prod)
            print(f"Estado= {a} ")
...
def crud():
    ...
    while flag:
        os.system('cls')
        menu_crud()
        opcion=input( "\n→ Ingrese la opción que desea usar, o V para volver al menú anterior: → " ).upper()
